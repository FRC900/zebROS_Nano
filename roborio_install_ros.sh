...
scp -P 22 ~/zebROS_Nano/roscore_roborio.tar.bz2 admin@$1:.
ssh -p 22 admin@$1 'cd / && tar -xjf ~/roscore_roborio.tar.bz2'
ssh -p 22 admin@$1 'rm ~/roscore_roborio.tar.bz2'
scp -P 22 ~/zebROS_Nano/os_detect.py admin@$1:/usr/lib/python2.7/site-packages/rospkg/
...
ssh -p 22 admin@$1 'mkdir -p zebROS_Nano/zebROS_ws/src'
ssh -p 22 admin@$1 'source /opt/ros/melodic/setup.bash && cd zebROS_Nano/zebROS_ws && catkin_make_isolated --install'

#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#
##################-----------------------------#################
#Edit /etc/ntp.conf to be a copy of ntp-server in zebROS_Nano#
scp -P 22 ~/zebROS_Nano/ntp-server.conf admin@$1:/etc/ntp.conf

#---------------------------------------------------------------#
#               to setup RTC on the rio
#   Plug the rtc into the i2c port on the rio while unpowered
#
scp -P 22 ~/zebROS_Nano/rtc-bq32k.ko admin@$1:.
...
scp -P 22 ~/zebROS_Nano/setupClock admin@$1:/etc/init.d/setupClock
...
# Set up ssh keys
scp -P 22 ~/zebROS_Nano/jetson_setup/roborio_dot_ssh.tar.bz2 admin@$1:.
...
scp -P 22 ~/zebROS_Nano/rio_bashrc.sh admin@$1:.
scp -P 22 ~/zebROS_Nano/zebROS_ws/ROSJetsonMaster.sh admin@$1:.

# Set up prereqs for deploy script
ssh -p 22 admin@$1 'mv ~/zebROS_Nano ~/zebROS_Nano.orig'
ssh -p 22 admin@$1 'ln -s ~/zebROS_Nano.orig ~/zebROS_Nano'
ssh -p 22 admin@$1 'mkdir -p ~/zebROS_Nano.prod/zebROS_ws'
ssh -p 22 admin@$1 'mkdir -p ~/zebROS_Nano.dev/zebROS_ws'
