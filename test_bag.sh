	...
	then
		...
		then
			...
			then
				...
				cp $var $HOME/zebROS_Nano/practice$(basename $var)
			...
			then
				...
				cp $HOME/zebROS_Nano/$practice_merge_file $HOME/zebROS_Nano/unmerged_${practice_merge_file}
				~/zebROS_Nano/zebROS_ws/src/rosbag_scripts/scripts/merge_bagfiles.py $practice_merge_file $var unmerged_$practice_merge_file -v
		...
		then 
			...
			then
				...
				cp $var $HOME/zebROS_Nano/match0$(basename $var)
				continue
			fi
			...
			then 
				...
				~/zebROS_Nano/zebROS_ws/src/rosbag_scripts/scripts/merge_bagfiles.py ${bag_name}_${count}.bag $var ${bag_name}*.bag -v
				rm ${bag_name}_$(( $count - 1)).bag
				rm ${bag_name}.bag
				count=$(( $count + 1 ))
			else 
				echo Renaming bag file to ${bag_name}.bag
				cp $var $HOME/zebROS_Nano/${bag_name}.bag
			fi
		else
			echo This does not have alliance data -- renaming to prematch${var}
			cp $var $HOME/zebROS_Nano/prematch$(basename $var)
		fi
	else
