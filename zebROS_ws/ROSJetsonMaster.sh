...
if [ -f /.dockerenv ] ; then
...
elif [ -f /home/ubuntu/zebROS_Nano/zebROS_ws/devel/setup.bash ] ; then
	...
	export ROS_IP=`ip route get 10.9.0.1 | sed 's/ via [[:digit:]]\+\.[[:digit:]]\+\.[[:digit:]]\+\.[[:digit:]]\+//' | sed 's/lo //' | head -1 | cut -d ' ' -f 5`
elif [ -f /home/ofugikawa/zebROS_Nano/zebROS_ws/devel/setup.bash ] ; then
    # Jetson-specific configuration
    echo "Sourcing Olivia / native Linux environment"
    source /home/ofugikawa/zebROS_Nano/zebROS_ws/devel/setup.bash
    ...
    source /opt/ros/melodic/setup.bash
    source $HOME/zebROS_Nano/zebROS_ws/devel/setup.bash
	export ROS_IP=`ip route get 10.9.0.1 | sed 's/ via [[:digit:]]\+\.[[:digit:]]\+\.[[:digit:]]\+\.[[:digit:]]\+//' | sed 's/lo //' | head -1 | cut -d ' ' -f 5`
fi
